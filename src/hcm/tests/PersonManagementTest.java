...
public class PersonManagementTest extends BaseTest{
	...
	public void assignDepartment() throws Exception{
		...
		welcome.clickNavigator("More...");
		clickNavigationLink("Person Management");
		
		String person, labelLocator, labelLocatorPath;

		while(getExcelData(input, assignee, "text").length()>0){
		
		//Search for person credentials here...
		person = getExcelData(input, assignee, "text");
		labelLocator = "Name";
		...
		//PeopleManagementPage task = new PeopleManagementPage(driver);
		takeScreenshot();
		...
		TaskUtilities.customWaitForElementVisibility("//td/a[text()='Person Management']", MAX_TIME_OUT);
		TaskUtilities.retryingFindClick(By.xpath("//td/a[text()='Person Management']"));
		...
		labelLocator = "Name";
		labelLocatorPath = TaskUtilities.retryingSearchInput(labelLocator);
		person = getExcelData(input-1, assignee, "text");
